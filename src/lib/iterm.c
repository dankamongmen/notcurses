...
int iterm_draw(const tinfo* ti, const ncpile *p, sprixel* s, fbuf* f, int y, int x){
  (void)ti;
  if(p){
      if(goto_location(p->nc, f, y, x, s->n)){
      return -1;
    }
  }
...
int iterm_blit(ncplane* n, int linesize, const void* data,
  ...
  unsigned int cols = bargs->u.pixel.spx->dimx;
  unsigned int rows = bargs->u.pixel.spx->dimy;
  ...
  scrub_tam_boundaries(tam, leny, lenx, ncplane_pile(s->n)->cellpxy, ncplane_pile(s->n)->cellpxx);
  if(plane_blit_sixel(s, &s->glyph, leny, lenx, parse_start, tam, SPRIXEL_INVALIDATED) < 0){
    if(!reuse){
      free(tam);
    }
    fbuf_free(&s->glyph);
